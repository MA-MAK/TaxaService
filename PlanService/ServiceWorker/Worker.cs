...
public class Worker : BackgroundService
{
    ...
    {
        _logger = logger;
        _planPath = configuration["plan"] ?? String.Empty;
        _mqHost = configuration["mqHost"] ?? "localhost";
    }
    ...
    public string SendPlan(Plan plan)
{
    try
    {
        if (plan == null)
        ...
        var filePath = Path.Combine("plan", uniqueFileName);
